...
var _cases = __webpack_require__(106);
...
var _selection = __webpack_require__(111);
...
function changeCase(methodName, node) {
    var method = _cases2.default[methodName],
        result = void 0,
        start = void 0,
        stop = void 0;
    ...
    if (node.element.nodeType === 3) node.element.nodeValue = result;else node.element.value = result;
...
if (typeof chrome !== 'undefined') {
    chrome.runtime.onMessage.addListener(function (request) {
        ...
        var methodName = request.value || false,
            selection = (0, _selection2.default)();
...
var _private = exports._private = void 0;
...
var _nonWordRegexp = __webpack_require__(107);

var _nonWordRegexp2 = _interopRequireDefault(_nonWordRegexp);

var _camelCaseRegexp = __webpack_require__(108);

var _camelCaseRegexp2 = _interopRequireDefault(_camelCaseRegexp);

var _camelCaseUpperRegexp = __webpack_require__(109);

var _camelCaseUpperRegexp2 = _interopRequireDefault(_camelCaseUpperRegexp);
...
function _noCase(value, replacement) {
    ...
    if (typeof replacement === 'string') {
        value = value.replace(_nonWordRegexp2.default, function (match, index, value) {
...
function titleCase(value) {
    return _noCase(value).replace(/^.| ./g, function (a) {
        return upperCase(a);
    });
}

function sentenceCase(value) {
    return upperCaseFirst(_noCase(value));
}
...
function getAttributes(element, include) {
    ...
    if (include && include.length && include.constructor === Array) {
        attributes = attributes.filter(function (attr) {
            return include.indexOf(attr.name) > -1;
        });
    }
    return attributes;
}
...
function activeElement(localDocument) {
    var element = (localDocument || document).activeElement,
        tagName = element.tagName.toLowerCase();
    if (tagName === 'iframe' || tagName === 'frame') return activeElement(element.contentDocument);
    return element;
}
...
function dispatchError() {
    ...
    if (element && element.tagName) {
        ...
        issue += getAttributes(element, ['id', 'class', 'name', 'type']).map(function (attr) {
            return attr.name + '="' + attr.value + '"';
        }).join(' ');
        issue = '<' + issue.trim() + '>';
    }
    prompt("An error occurred. Publish the following informations about issue, on the extension page.", window.location.href + issue);
}
...
function rangeTextNodes(range) {
    var node = range && range.start || null,
...
function interpolateRange(range) {
    if (range === false) {
        return [];
    }
    ...
    for (var i = 0; i < nodesLength; i++) {
        var node = nodes[i],
            text = node.nodeType === 3 ? node.nodeValue : node.value,
            start = i == 0 ? range.offset[0] : 0,
            stop = i == nodesLength - 1 ? range.offset[1] : text.length;
        output.push({
            element: node,
            range: [start, stop],
            text: text
...
function selectionRange(localWindow, localDocument) {
    ...
    var element = localDocument.activeElement,
        ...
        try {
            return {
                empty: function empty() {
                    return localWindow.getSelection().empty();
                },
                start: element,
                end: element,
                offset: [element.selectionStart, element.selectionEnd]
            };
        } catch (e) {
            return false;
        }
    ...
    if (tagName === 'iframe' || tagName === 'frame') {
        return selectionRange(element.contentWindow, element.contentDocument);
    }
...
function selectionText() {
    var emptyAfter = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
